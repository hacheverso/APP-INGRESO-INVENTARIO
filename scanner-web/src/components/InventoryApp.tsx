...
export default function InventoryScannerApp({ initialView = 'SCANNER' }: { initialView?: AppView }) {
    ...
    useEffect(() => {
        const onPop = () => {
            ...
            const match = (Object.entries(VIEW_PATHS).find(([, path]) => path === p)?.[0] || 'SCANNER') as AppView;
            setView(match);
        };
    ...
    const stats = (() => {
        const copSessions = savedSessions.filter(s => s.monedaBase === 'COP');
        const usdSessions = savedSessions.filter(s => s.monedaBase !== 'COP');

        const totalCop = copSessions.reduce((a, s) => a + (s.costoTotalCOP || 0), 0);
        const totalUsd = usdSessions.reduce((a, s) => a + (s.costoTotalCOP || 0), 0); // en sesiones USD ese campo guarda dólares
        const totalUnidades = savedSessions.reduce((a, s) => a + (s.totalUnidades || 0), 0);
        const unidadesCop = copSessions.reduce((a, s) => a + (s.totalUnidades || 0), 0);
        const facturadas = savedSessions.filter(s => s.holdedInvoiceNum).length;

        const proveedores = new Set(savedSessions.map(s => (s.proveedor || '').trim()).filter(Boolean));

        // Agrupaciones por registro (solo montos COP de sesiones COP para no mezclar monedas)
        const porProducto = new Map<string, { nombre: string; sku: string; unidades: number; cop: number }>();
        const porProveedor = new Map<string, { sesiones: Set<string>; unidades: number; cop: number }>();
        const porCategoria = new Map<string, { unidades: number; cop: number }>();

        savedSessions.forEach(s => {
            const esCop = s.monedaBase === 'COP';
            ...
            s.records.forEach(r => {
                ...
                const p = porProducto.get(r.UPC) || { nombre: r.Nombre, sku: r.SKU || '', unidades: 0, cop: 0 };
                p.unidades += r.Cantidad || 0;
                p.cop += copVal;
                if (!p.nombre && r.Nombre) p.nombre = r.Nombre;
                porProducto.set(r.UPC, p);

                const catName = (productDB[r.UPC]?.CATEGORIA || 'Sin categoría').trim() || 'Sin categoría';
                const cat = porCategoria.get(catName) || { unidades: 0, cop: 0 };
                cat.unidades += r.Cantidad || 0;
                cat.cop += copVal;
                porCategoria.set(catName, cat);
            });
            porProveedor.set(provName, prov);
        });

        const topProductos = Array.from(porProducto.entries())
            .map(([upc, p]) => ({ upc, ...p }))
            .sort((a, b) => b.cop - a.cop || b.unidades - a.unidades)
            .slice(0, 10);
        const topProveedores = Array.from(porProveedor.entries())
            .map(([nombre, p]) => ({ nombre, sesiones: p.sesiones.size, unidades: p.unidades, cop: p.cop }))
            .sort((a, b) => b.cop - a.cop || b.unidades - a.unidades)
            .slice(0, 8);
        ...
        return {
            totalCop, totalUsd, totalUnidades, facturadas,
            numSesiones: savedSessions.length,
            numProductos: Object.keys(productDB).length,
            numProveedores: proveedores.size,
            costoPromedioUnidad: unidadesCop > 0 ? totalCop / unidadesCop : 0,
            topProductos, topProveedores, topCategorias,
            maxProductoCop: Math.max(1, ...topProductos.map(p => p.cop)),
            ultimosIngresos: savedSessions.slice(0, 6),
        };
    })();
        ...
        <div className="min-h-[100vh] flex flex-col font-sans bg-page text-ink transition-colors duration-300">
            ...
            {deleteConfirm.id && (
                <div className="fixed inset-0 bg-ink/50 backdrop-blur-sm flex items-center justify-center z-[100] animate-in fade-in duration-200" onClick={() => setDeleteConfirm({ id: null, type: 'record' })}>
                    <div className="glass-strong rounded-3xl w-[400px] overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            {showNewProductModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink/40 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="glass-strong rounded-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            {showSheetsUrlModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink/45 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setShowSheetsUrlModal(false)}>
                    <div className="glass-strong rounded-3xl w-[520px] overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                        {/* Header */}
            ...
            <header className="glass rounded-3xl mx-4 md:mx-6 2xl:mx-10 mt-4 px-6 lg:px-8 py-4 flex flex-wrap items-center justify-between z-10 transition-colors duration-300 gap-4">
                ...
                <div className="flex flex-wrap items-center justify-end gap-3 z-20 relative">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="flex bg-field rounded-xl border border-line overflow-hidden shadow-[0_2px_10px_rgba(23,28,20,0.06)]">
                            ...
                            <button onClick={() => navigateTo('STATS')} className={`flex items-center gap-2 px-4 py-2 font-black text-[10px] sm:text-xs uppercase tracking-wider transition-all border-l border-line ${view === 'STATS' ? 'bg-brand-blue/20 text-brand-blue' : 'text-muted hover:text-ink-soft hover:bg-ink/5'}`} title="Estadísticas e indicadores">
                    ...
                    <div className="flex-1 flex flex-col gap-6 w-full animate-in fade-in duration-300 overflow-y-auto pr-2 custom-scrollbar h-[800px] xl:h-[calc(100vh-140px)] min-h-0">
                        ...
                        {savedSessions.length === 0 ? (
                            <div className="flex-1 flex flex-col items-center justify-center text-center opacity-40 py-20">
                                <BarChart3 size={48} className="text-faint mb-4" />
                                <p className="text-muted font-bold uppercase tracking-widest">Aún no hay ingresos guardados para analizar</p>
                            </div>
                        ) : (
                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 pb-12">
                            {/* Top productos por inversión */}
                            <div className="glass rounded-3xl overflow-hidden xl:col-span-2">
                                <div className="px-6 py-4 border-b border-line flex items-center gap-2">
                                    <PackageCheck size={16} className="text-brand-green-ink" />
                                    <h3 className="font-black text-ink text-sm uppercase tracking-wider">Top productos por inversión</h3>
                                </div>
                                    ...
                                    <table className="w-full text-left text-sm">
                                        <thead>
                                            <tr className="text-muted uppercase tracking-wider text-[10px] border-b border-line">
                                                ...
                                                <th className="px-4 py-3 font-black text-right">Unidades</th>
                                                <th className="px-4 py-3 font-black text-right">Inversión (COP)</th>
                                                <th className="px-6 py-3 font-black w-[30%]">Participación</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-line/60">
                                            {stats.topProductos.map(p => (
                                                <tr key={p.upc} className="hover:bg-ink/5 transition-colors">
                                                    <td className="px-6 py-3">
                                                        <span className="font-bold text-ink block truncate max-w-[380px]" title={p.nombre}>{p.nombre}</span>
                                                        <span className="text-[10px] font-mono text-muted">{p.sku || p.upc}</span>
                                                    </td>
                                                    <td className="px-4 py-3 text-right font-mono font-bold text-ink-soft">{p.unidades.toLocaleString('es-CO')}</td>
                                                    <td className="px-4 py-3 text-right font-mono font-black text-brand-green-ink">{formatMoney(p.cop, 'COP')}</td>
                                                    <td className="px-6 py-3">
                                                        <div className="h-2.5 bg-ink/5 rounded-full overflow-hidden">
                                                            <div className="h-full rounded-full bg-brand-green border border-brand-green-deep/40" style={{ width: `${Math.max(2, (p.cop / stats.maxProductoCop) * 100)}%` }} />
                                                        </div>
                                                    </td>
                                                </tr>
                                            ))}
                            ...
                            <div className="glass rounded-3xl overflow-hidden">
                                <div className="px-6 py-4 border-b border-line flex items-center gap-2">
                                    ...
                                    <h3 className="font-black text-ink text-sm uppercase tracking-wider">Inversión por proveedor</h3>
                                </div>
                                <table className="w-full text-left text-sm">
                                    <tbody className="divide-y divide-line/60">
                                        {stats.topProveedores.map(p => (
                                            <tr key={p.nombre} className="hover:bg-ink/5 transition-colors">
                                                <td className="px-6 py-3">
                                                    <span className="font-bold text-ink">{p.nombre}</span>
                                                    <span className="block text-[10px] text-muted font-bold uppercase tracking-wider">{p.sesiones} {p.sesiones === 1 ? 'ingreso' : 'ingresos'} · {p.unidades.toLocaleString('es-CO')} und</span>
                                                </td>
                                                <td className="px-6 py-3 text-right font-mono font-black text-brand-green-ink whitespace-nowrap">{formatMoney(p.cop, 'COP')}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                            ...
                            <div className="glass rounded-3xl overflow-hidden">
                                <div className="px-6 py-4 border-b border-line flex items-center gap-2">
                                    <Tags size={16} className="text-brand-green-ink" />
                                    <h3 className="font-black text-ink text-sm uppercase tracking-wider">Inversión por categoría</h3>
                                </div>
                                <table className="w-full text-left text-sm">
                                    <tbody className="divide-y divide-line/60">
                                        {stats.topCategorias.map(c => (
                                            <tr key={c.nombre} className="hover:bg-ink/5 transition-colors">
                                                <td className="px-6 py-3">
                                                    <span className="font-bold text-ink">{c.nombre}</span>
                                                    <span className="block text-[10px] text-muted font-bold uppercase tracking-wider">{c.unidades.toLocaleString('es-CO')} und</span>
                                                </td>
                                                <td className="px-6 py-3 text-right font-mono font-black text-brand-green-ink whitespace-nowrap">{formatMoney(c.cop, 'COP')}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                            ...
                            <div className="glass rounded-3xl overflow-hidden xl:col-span-2">
                                <div className="px-6 py-4 border-b border-line flex items-center gap-2">
                                    ...
                                    <h3 className="font-black text-ink text-sm uppercase tracking-wider">Últimos ingresos</h3>
                                </div>
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left text-sm">
                                        <thead>
                                            <tr className="text-muted uppercase tracking-wider text-[10px] border-b border-line">
                                                <th className="px-6 py-3 font-black">Lote</th>
                                                <th className="px-4 py-3 font-black">Proveedor</th>
                                                <th className="px-4 py-3 font-black text-right">Unidades</th>
                                                <th className="px-4 py-3 font-black text-right">Valor</th>
                                                <th className="px-6 py-3 font-black text-right">Holded</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-line/60">
                                            {stats.ultimosIngresos.map(s => (
                                                <tr key={s.id} className="hover:bg-ink/5 transition-colors cursor-pointer" onClick={() => navigateTo('HISTORY')}>
                                                    <td className="px-6 py-3 font-mono font-black text-ink">{s.lote}</td>
                                                    <td className="px-4 py-3 font-bold text-ink-soft">{s.proveedor || '---'}</td>
                                                    <td className="px-4 py-3 text-right font-mono font-bold text-ink-soft">{s.totalUnidades}</td>
                                                    <td className="px-4 py-3 text-right font-mono font-black text-brand-green-ink whitespace-nowrap">{formatMoney(s.costoTotalCOP, s.monedaBase)}</td>
                                                    <td className="px-6 py-3 text-right">
                                                        {s.holdedInvoiceNum ? (
                                                            <span className="inline-flex items-center gap-1 text-[10px] font-black text-brand-green-ink bg-brand-green/10 border border-brand-green/40 px-2 py-0.5 rounded-md uppercase tracking-wider"><CheckCircle size={10} /> {s.holdedInvoiceNum}</span>
                                                        ) : (
                                                            <span className="text-[10px] font-bold text-faint uppercase tracking-wider">Sin factura</span>
                                                        )}
                                                    </td>
                                                </tr>
                                            ))}
                                ...
                                {savedSessions.map(session => (
                                    <div key={session.id} className="glass rounded-3xl p-6 flex flex-col gap-4 hover:shadow-[0_14px_40px_rgba(58,82,218,0.16)] transition-all group">
                                        <div className="flex justify-between items-start gap-3">
                                            ...
                                            {session.proveedor && (
                                                <span className="shrink-0 text-[10px] bg-brand-blue/10 border border-brand-blue/30 text-brand-blue px-2.5 py-1 rounded-lg font-black uppercase tracking-wider max-w-[45%] truncate" title={session.proveedor}>
                                                    {session.proveedor}
                                                </span>
                                            )}
                                        </div>
                    ...
                    <div className="flex-1 flex flex-col gap-6 w-full animate-in fade-in duration-300 overflow-y-auto pr-2 custom-scrollbar h-[800px] xl:h-[calc(100vh-140px)] min-h-0">
                        <div className="flex items-center justify-between mb-2 gap-4 flex-wrap">
                            <div className="flex items-center gap-3 text-ink">
                                ...
                                <h2 className="font-display text-2xl tracking-[0.08em] uppercase">Catálogo de Productos Maestro</h2>
                            </div>
                        ...
                        {Object.keys(productDB).length === 0 ? (
                            ...
                            <div className="glass rounded-3xl overflow-hidden flex-shrink-0">
                    ...
                    <>
                        ...
                        <div className="w-full xl:w-[600px] flex flex-col gap-6 flex-shrink-0 animate-in slide-in-from-left-4 duration-300 xl:h-full">
                            ...
                            <div className={`flex-1 glass rounded-3xl p-6 flex flex-col items-center justify-center relative min-h-[440px] transition-all duration-300 group overflow-hidden ${isFlashing && scanStatus === 'success' ? 'ring-2 ring-brand-green/60 bg-brand-green/10' : ''} ${isFlashing && scanStatus === 'error' ? 'ring-2 ring-red-500/50 bg-red-500/10' : ''}`}>
                                ...
                                {matchedProduct ? (
                                    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none p-6 z-30 bg-white/70 backdrop-blur-xl rounded-3xl overflow-y-auto custom-scrollbar">
                                    ...
                                    <div className="absolute inset-0 flex flex-col items-center justify-center p-6 z-30 bg-white/70 backdrop-blur-xl rounded-3xl animate-in slide-in-from-bottom-4 duration-300">
                        ...
                        <div className="flex-1 flex flex-col glass rounded-3xl relative overflow-hidden h-[600px] lg:h-full lg:max-h-[calc(100vh-120px)] min-h-0 shrink-0">

                            {/* Header (Top) */}
                            <div className="flex justify-between items-center px-8 py-6 border-b border-line/70 bg-transparent shrink-0">
